...
remove_block(ospfs_inode_t *oi)
{
	...
	// in indirect block
	  ...
	  uint32_t *in = ospfs_block(oi->oi_indirect);
	  free_block(in[direct_index(n)]);
	  in[direct_index(n)] = 0;
	...
	// in indirect2 block
	  ...
	  uint32_t *in2 = ospfs_block(oi->oi_indirect2);
	  uint32_t *in = ospfs_block(in2[indir_index(n)]);

	  free_block(in[indir_index(n)]);
	  in[direct_index(n)] = 0;

	  if(direct_index(n) == 0) {
	    // remove indirect block
	    free_block(in2[indir_index(n)]);
	    in2[indir_index(n)] = 0;
	  }
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
	...
	// check name lengths
	if (OSPFS_MAXNAMELEN < dentry->d_name.len || OSPFS_MAXSYMLINKLEN < strlen(symname))
	  {
	    return -ENAMETOOLONG;
	  }
	...
	search_inode->oi_ftype = OSPFS_FTYPE_SYMLINK;
	search_inode->oi_nlink = 1;
	strncpy(search_inode->oi_symlink, symname, strlen(symname));
	search_inode->oi_size = strlen(symname);
